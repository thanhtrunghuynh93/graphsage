...
def main(args):
    G_data = json.load(open(args.input + "/graphsage/" + args.prefix + "-G.json"))
...
def save_new_graph(G, input_dir, output_dir, prefix):
    ...
    nx.write_edgelist(G, path = output_dir + "/edgelist/" + prefix + ".edgelist" , delimiter=" ", data=['weight'])

    output_prefix = output_dir + "/graphsage/" + prefix
    print("Saving new class map")
    input_dir += "/graphsage/" + prefix
    class_map_file = Path(input_dir + "-class_map.json")
    if class_map_file.is_file():
        class_map = json.load(open(input_dir + "-class_map.json")) # id to class
        new_class_map = {node: class_map[node] for node in nodes}
        with open(output_prefix + '-class_map.json', 'w') as outfile:
    ...
    old_idmap = json.load(open(input_dir + "-id_map.json"))
