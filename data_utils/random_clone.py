...
def add_and_remove_edges(G, p_new_connection, p_remove_connection, num_add=10, num_remove=10):
    '''
    for each node,
    ...
    for node in G.nodes():
        ...
        if len(connected) and count_rm <= num_remove: # only try if an edge exists to remove
...
def main(args):
    ...
    H = G.copy()
    n = len(G.nodes())
    rem_edges, new_edges = add_and_remove_edges(H, args.padd, args.pdel, int(args.nadd * n), int(args.ndel * n))
    print("Remove {0} and add {1} edges".format(len(rem_edges), len(new_edges)))
    data = json_graph.node_link_data(H)
    s = json.dumps(data,  indent=4, sort_keys=True)
    print(nx.info(H))

    args.output += "clone,p={0},n={1}".format(args.pdel, args.ndel)
    edgelist_dir = args.output + "/" + args.prefix + ".edgelist"
